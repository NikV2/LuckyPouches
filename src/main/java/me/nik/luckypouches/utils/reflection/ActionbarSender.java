...
public final class ActionbarSender {
    ...
    public static void sendActionbar(Player player, String text) {
        try {
            ...
            player.spigot().sendMessage(ChatMessageType.ACTION_BAR, new TextComponent(ChatUtils.format(text)));

        } catch (NoSuchMethodError e) {

            try {

                Object chatBaseComponent = ReflectionUtils.getMethod(ReflectionUtils.getNMSClass("IChatBaseComponent$ChatSerializer"), "a", String.class).invoke(null, "{\"text\":\"" + ChatUtils.format(text) + "\"}");
